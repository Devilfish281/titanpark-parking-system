...
## Windows and Poetry Database Setup and Run the Server

### 1. Easiest path (no `psql` needed): Docker + Alembic (Option A)

If you’re ok using Docker, you can **ignore that `psql` command entirely** and just follow the Docker path from the README, adjusted for Windows/PowerShell.

### Step 1 – Make sure Docker Desktop is installed and running

- Install Docker Desktop for Windows if you don’t have it.
- Start Docker Desktop and wait until it says it’s running.

You can test it in PowerShell with:

```powershell
docker version
```

### Step 2 – Go to the project folder and activate venv (Poetry)

Activate the Virtual Environment – one-step process.
In PowerShell:

```powershell
cd C:\Users\Me\Documents\Python\CPSC491\Projects\titanpark-parking-system
...
(If you already did the venv + poetry install steps earlier, just do the `Activate.ps1` part.)
...
- Pull a Postgres image if needed
- Start a database container named something like `titanpark-parking-system-db`
...
### Step 4 – Point `DATABASE_URL` at that container

In the same PowerShell where your venv is active:

```powershell
$env:DATABASE_URL = "postgresql://postgres:titanpark@localhost:5432/titanpark_parking_system"
alembic upgrade head
```
...
```text
postgresql://postgres:titanpark@localhost:5432/titanpark_parking_system
```

### Step 5 – Run Alembic migrations (create tables + seed parking structures)
...
- On Windows/PowerShell you use `$env:VAR = "value"` instead of `export VAR=...`.
- `alembic upgrade head` uses `alembic.ini` in the project root and the `DATABASE_URL` you just set.

This will:

- Create the schema
- Insert the default parking structures
...
Still in the venv, from the project root:
...
You should see the redirect to `/docs` and the interactive API docs.

---

**Verify it works**:

- Visit `http://127.0.0.1:8000` → should redirect to `/docs`.
- In the Swagger UI, call `GET /parking_data/all` to see live parking data.
